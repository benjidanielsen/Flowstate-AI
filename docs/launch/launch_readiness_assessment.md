...
- **Operational hardening:** Lint, unit, integration, and Playwright suites now pass locally, but there is still no automated smoke/regression pipeline. CI must provision dependencies and enforce these checks before go-live.【F:backend/src/middleware/correlationId.ts†L1-L40】【F:backend/src/utils/dbOptimizer.ts†L1-L160】
- **End-to-end environment parity:** Playwright coverage authenticates and passes locally, yet CI/staging environments must still provision browser binaries (for example with `npx playwright install --with-deps chromium`) before the suite can gate releases reliably.【F:e2e/tests/01-customer-creation.spec.ts†L9-L104】【F:e2e/utils/auth.ts†L1-L140】
