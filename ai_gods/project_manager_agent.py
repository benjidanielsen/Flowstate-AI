...
class ProjectManagerAgent(BaseAgent):
    def __init__(
        ...
        redis_port: int = 6379,
        db_path: str = "/tmp/agent_state.db",
    ):
        super().__init__(
            agent_id,
            agent_name,
        ...
        self.subscribe_to_channel("project_manager_tasks", self.handle_message)
    ...
    def process_message(self, message_data: Dict[str, Any]):
        ...
        if message_type == "agent_status_update":
            logger.info(
                f"PM received status update from {sender_name} ({sender_id}): {payload.get('status')}, Task: {payload.get('current_task')}, Progress: {payload.get('progress')}% "
            )
        elif message_type == "task_request":
            requested_task = payload.get("task_description")
            logger.info(
                f"PM received task request from {sender_name}: {requested_task}"
            )
            ...
            logger.info(
                f"PM received task completion from {sender_name}: {completed_task}"
            )
        elif message_type == "feature_implementation":
            self.delegate_task(
                task_description=payload.get("description"),
                assignee_id=payload.get("details", {}).get(
                    "recommended_agent", "frontend-developer"
                ),
                action="implement_feature",
            )
        elif message_type == "autofix_request":
            if self.can_auto_approve():
                self.delegate_task(
                    task_description="Full system autofix",
                    assignee_id="FixerAI",
                    action="autofix",
                )
            else:
                logger.warning(
                    "Autofix request received, but full autonomy is not enabled. Manual approval required."
                )

    def delegate_task(
        self, task_description: str, assignee_id: str, action: str = "execute"
    ):
        ...
        if "autofix" in task_description.lower():
            assignee_id = "FixerAI"
            action = "autofix"
        elif action in ["commit", "push", "pull"]:
            assignee_id = "GitAgent"
        elif "vscode" in action:
            assignee_id = "VSCodeAgent"
        ...
        logger.info(
            f"Project Manager delegated task '{task_description}' to {assignee_id} with action '{action}'."
        )
        self._update_agent_state(
            current_task=f"Delegating: {task_description}", progress=50
        )

    def monitor_progress(self):
        logger.info(
            f"Project Manager {self.agent_name} is monitoring overall project progress."
        )
        all_agents_state = self.get_all_agent_states()
        for agent_state in all_agents_state:
            logger.info(
                f"Agent {agent_state.get('agent_name')} ({agent_state.get('role')}) - Status: {agent_state.get('status')}, Task: {agent_state.get('current_task')}, Progress: {agent_state.get('progress')}% "
            )
...
if __name__ == "__main__":
    ...
    class SimpleAgent(BaseAgent):
        ...
        def request_task(self, task_description: str):
            ...
            logger.info(
                f"Simple Agent {self.agent_name} requested task: {task_description}"
            )
        ...
        def process_message(self, message_data: dict):
            ...
            if message_data.get("type") == "task_assignment":
                ...
                for i in range(1, 6):
                    time.sleep(0.5)
                    self._update_agent_state(
                        current_task=task_description, progress=i * 20
                    )
                self._update_agent_state(
                ...
                logger.info(
                    f"Simple Agent {self.agent_name} completed task: {task_description}"
                )
