...
class ProjectManagerV2:
    ...
    async def _load_state(self):
        ...
        try:
            ...
            async with self.db.execute('SELECT * FROM agents') as cursor:
                async for row in cursor:
                    ...
                    self.agents[agent_id] = AgentPerformance(
                        ...
                        last_heartbeat=datetime.fromisoformat(row[10]) if row[10] else datetime.now()
                    )
            
            # Load tasks
            async with self.db.execute('SELECT * FROM tasks WHERE status NOT IN ("COMPLETED", "FAILED", "CANCELLED")') as cursor:
                async for row in cursor:
                    task_id = row[0]
                    task = Task(
                        id=task_id,
            ...
            logger.info(f"✅ Loaded {len(self.agents)} agents and {len(self.tasks)} active tasks from database")
            
        except Exception as e:
            logger.error(f"❌ Error loading state: {e}")
    
    async def register_agent(self, agent_id: str, capabilities: List[str], max_load: int = 3):
        ...
        try:
            ...
            if self.redis_available:
                await self.redis.publish('agent_registry', json.dumps({
                    'action': 'register',
                    'agent_id': agent_id,
                    'capabilities': capabilities,
    ...
    async def _has_circular_dependency(self, task_id: str, visited: Set[str] = None) -> bool:
    ...
    async def assign_next_task(self) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            await self.db.execute('''
                ...
                WHERE agent_id = ?
            ''', (self.agents[best_agent].current_load, self.agents[best_agent].status.value, best_agent))
            ...
            # Publish to Redis
            if self.redis_available:
                await self.redis.publish('task_assignments', json.dumps({
                    'task_id': task_id,
                    'task_name': task.name,
    ...
    async def complete_task(self, task_id: str, success: bool = True):
        ...
        try:
            ...
            if task.assigned_to and task.assigned_to in self.agents:
                ...
                # Update success rate
                agent.success_rate = (agent.tasks_completed / total_tasks) * 100 if total_tasks > 0 else 100.0
                ...
                ''', (
                    ...
                    agent.success_rate, agent.current_load, agent.status.value, task.assigned_to
                ))
            ...
            logger.info(f"✅ Task completed: {task.name} (Duration: {task.actual_duration} min, Success: {success})")
    ...
    async def _check_unblocked_tasks(self):
        ...
        for task_id, task in self.tasks.items():
            if task.status == TaskStatus.BLOCKED and await self._are_dependencies_met(task):
                task.status = TaskStatus.QUEUED
                await self.task_priority_queue.put((task.priority.value, task_id))
