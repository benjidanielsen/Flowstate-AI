...
class InteractionService {
    ...
    async create(interaction) {
        ...
        try {
            ...
            const result = await client.query(`INSERT INTO interactions (id, customer_id, type, summary, notes, interaction_date, created_at, updated_at)
    ...
    async getByCustomerId(customerId) {
        ...
        try {
            const pool = database_1.default.getInstance().getPool();
            client = await pool.connect();
            const result = await client.query(`SELECT * FROM interactions WHERE customer_id = $1 ORDER BY interaction_date DESC`, [customerId]);
    ...
    async update(id, updates) {
        ...
        try {
            ...
            if (updates.interaction_date) {
                fields.push('interaction_date = $' + (fields.length + 1));
            ...
            values.push(now);
            values.push(id);
            const result = await client.query(`UPDATE interactions SET ${fields.join(', ')} WHERE id = $${values.length} RETURNING *`, values);
    ...
    async delete(id) {
        ...
        try {
            const pool = database_1.default.getInstance().getPool();
            client = await pool.connect();
            const result = await client.query(`DELETE FROM interactions WHERE id = $1 RETURNING customer_id`, [id]);
    ...
    async getAllInteractions() {
        ...
        try {
            const pool = database_1.default.getInstance().getPool();
            client = await pool.connect();
            const result = await client.query(`SELECT * FROM interactions ORDER BY timestamp DESC`);
            return result.rows.map(row => ({
                id: row.id,
                customer_id: row.customer_id,
                type: row.type,
    ...
    async getInteractionSummaryByType() {
        let client = null;
        try {
            const pool = database_1.default.getInstance().getPool();
            client = await pool.connect();
            const result = await client.query(`SELECT type, COUNT(*) as count FROM interactions GROUP BY type`);
