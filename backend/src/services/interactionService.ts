declare module '../database' {
  const DatabaseManager: any;
  export default DatabaseManager;
}
import DatabaseManager from '../database';
// Local Interaction type to avoid missing external module; replace with a shared type file if desired.
...
class EventLogService {
  logEvent(eventType: string, data: any, customerId?: string) {
    // Lightweight in-file implementation to avoid missing-module errors.
    // Replace with the real implementation or move to a separate module as needed.
    try {
      console.log('Event:', eventType, { ...data, customerId });
    } catch {
      // prevent logging errors from affecting main flow
    }
  }
}
...
export class InteractionService {
  ...
  async getInteractionsByCustomer(customerId: string): Promise<Interaction[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      `, [customerId], (err: Error | null, rows: any[]) => {
        if (err) {
          reject(err);
        } else {
  ...
  async getInteractionById(id: string): Promise<Interaction | null> {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM interactions WHERE id = ?', [id], (err: Error | null, row: any) => {
        if (err) {
          reject(err);
        } else if (!row) {
          resolve(null);
  ...
  async deleteInteraction(id: string): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      db.run('DELETE FROM interactions WHERE id = ?', [id], function(this: any, err: Error | null) {
        if (err) {
          reject(err);
        } else {
  ...
  async getUpcomingInteractions(limit: number = 20): Promise<Interaction[]> {
    ...
    return new Promise((resolve, reject) => {
      db.all(`
        ...
        LIMIT ?
      `, [now, limit], (err: Error | null, rows: any[]) => {
        if (err) {
          reject(err);
        } else {
          const interactions = rows.map(row => ({
