...
class ReminderService:
    ...
    async def create_reminder(self, customer_id: str, reminder_type: str, message: str, scheduled_for: datetime):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, 0, ?)
        """, (reminder_id, customer_id, reminder_type, message, scheduled_for.isoformat(), datetime.now(timezone.utc).isoformat()))
    ...
    async def _create_follow_up_action(self, reminder: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if reminder_type in follow_up_intervals:
            next_scheduled = datetime.now(timezone.utc) + follow_up_intervals[reminder_type]
    ...
    async def _create_interaction_record(self, customer_id: str, content: str):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, 1)
        """, (interaction_id, customer_id, 'reminder', content, datetime.now(timezone.utc).isoformat()))
